...
import { createGame, step, press, aim, holdLife, readout, predictLanding, APERTURE } from '../game.js';
import { deck, deckSummary } from '../cards.js';
import { pointerFraction, WEB_KEYS, PANEL_KEYS } from '../input.js';
...
console.log('\n— the pointer and the profiles —');

// The web build steers with a pointer, and the mapping from a screen x to a
// fraction of the field is the piece of it that can be wrong silently: the
// canvas is scaled by CSS and inset for the panel casing, so getting it wrong
// drifts the catcher from the cursor by that margin at the edges. It is a pure
// function precisely so it can be checked here rather than by waving a mouse at
// a pane that may not even have a box.
{
  const rect = { left: 100, width: 512 };     // drawn at half scale
  const field = { x: 22, w: 980 };
  const at = (canvasX) => pointerFraction(100 + canvasX * 0.5, rect, 1024, field);
  const near = (a, b) => Math.abs(a - b) < 1e-9;
  if (!near(at(22), 0)) fail(`the field's left edge maps to ${at(22)}, not 0`);
  if (!near(at(512), 0.5)) fail(`the field's centre maps to ${at(512)}, not 0.5`);
  if (!near(at(1002), 1)) fail(`the field's right edge maps to ${at(1002)}, not 1`);
  if (at(-500) !== 0 || at(5000) !== 1) fail('the pointer is not clamped to the field');
  if (pointerFraction(50, { left: 0, width: 0 }, 1024, field) !== null) {
    fail('an element with no box should give null, not a division by zero');
  }
  ok('the pointer maps the field edge to edge, clamps outside it, and gives null with no box');
}

// Escape must reach exactly one handler. app.js binds it, because what it means
// depends on the phase; having it in the key map as well meant one press opened
// the deck and resumed from it in the same keystroke, and nothing happened.
if ('Escape' in WEB_KEYS) fail('Escape is in WEB_KEYS as well as app.js, so it fires twice');
for (const verb of ['L', 'C', 'R']) {
  if (!Object.values(WEB_KEYS).includes(verb)) fail(`the web profile has no key for ${verb}`);
  if (!Object.values(PANEL_KEYS).includes(verb)) fail(`the panel profile has no key for ${verb}`);
}
for (const verb of ['CCW', 'CW', 'PRESS']) {
  if (!Object.values(PANEL_KEYS).includes(verb)) fail(`the panel profile has no key for ${verb}`);
}
ok('both profiles reach all three dials, and Escape is bound in one place only');

// Aiming is the pointer's verb and must land the catcher where it was told,
// inside the aperture's own limits.
{
  const h = createGame(2);
  press(h, 'PRESS');
  aim(h, 0.77);
  if (Math.abs(h.catcher.tx - 0.77) > 1e-9) fail(`aim(0.77) set tx to ${h.catcher.tx}`);
  aim(h, -5);
  if (h.catcher.tx < APERTURE - 1e-9) fail(`aim past the left edge set tx to ${h.catcher.tx}`);
  aim(h, 5);
  if (h.catcher.tx > 1 - APERTURE + 1e-9) fail(`aim past the right edge set tx to ${h.catcher.tx}`);
  aim(h, NaN);
  if (!Number.isFinite(h.catcher.tx)) fail('a non-finite aim reached the catcher');
  ok('aim lands where it is told and stays inside the field');
}
