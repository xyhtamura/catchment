...
/** Point the catcher at a fraction of the field. This is the pointer's verb,
 *  and it sets the same target the encoder steps toward, so the catcher still
 *  glides and the two controls feel like one. */
export function aim(g, x) {
  if (g.phase !== 'run' || !Number.isFinite(x)) return;
  g.catcher.tx = clamp(x, APERTURE, 1 - APERTURE);
}
