...
/** What to call each verb on the page being drawn. The panel has three
 *  labelled buttons and an encoder; the web has a pointer and a left hand. The
 *  legend has to say whichever is true, or the first thing a player reads is
 *  wrong. `L.controls` is set by app.js from the page. */
const KEYCAP = {
  panel: { L: 'L', C: 'C', R: 'R' },
  web: { L: 'A', C: 'S', R: 'D' },
};
const cap = (L, verb) => (KEYCAP[L.controls] || KEYCAP.web)[verb];
const isPanel = (L) => L.controls === 'panel';
...
function drawMenu(ctx, g, L) {
  ...
  for (let i = 0; i < deck.length; i++) {
    ...
    if (focused) continue;
    drawCard(ctx, L, deck[i], cx + d * (CARD_W + CARD_GAP), top, 0.9, 0.30);
  }
  const d0 = g.menu.at - g.menu.slide;
  drawCard(ctx, L, deck[g.menu.at], cx + d0 * (CARD_W + CARD_GAP), top, 1, 1);
  ...
  ctx.fillText(isPanel(L)
    ? 'turn to move through the deck   ·   C resumes'
    : 'scroll to move through the deck   ·   Esc resumes', cx, L.bottom - 8);
}

function drawCard(ctx, L, card, cx, top, scale, alpha) {
  ...
  const verb = isPanel(L) ? 'press' : 'click';
  const foot = card.action === 'restart' ? `${verb} to restart` : `${verb} to resume`;
  font(ctx, 12, '700');
...
function drawApertureBand(ctx, g, L) {
  ...
  ctx.fillText(isPanel(L) ? 'encoder moves the catcher' : 'the mouse moves the catcher',
    L.x + L.w - 14, L.apLabel);
}
...
function chargeDial(ctx, L, x, g) {
  dialHead(ctx, L, x, cap(L, 'L'), 'charge · colour');
  AXES.Q.forEach((q, i) => {
...
function parityDial(ctx, L, x, g) {
  dialHead(ctx, L, x, cap(L, 'C'), 'parity · fill');
  AXES.P.forEach((p, i) => {
...
function spinDial(ctx, L, x, g) {
  dialHead(ctx, L, x, cap(L, 'R'), 'spin · shape');
  AXES.J.forEach((j, i) => {
...
function drawTitle(ctx, g, L) {
  ...
  font(ctx, 13, '600'); ctx.fillStyle = C.mute;
  ctx.fillText(isPanel(L)
    ? 'encoder moves the catcher  ·  press to begin'
    : 'the mouse moves the catcher  ·  click to begin', cx, L.fieldY + 258);
